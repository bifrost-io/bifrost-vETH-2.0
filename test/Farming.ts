...
function parseUserInfo({ amount, rewardDebt, pending, nextClaimableBlock }: MaybeInfo): UserInfo {
  return {
    amount,
    rewardDebt,
    pending,
...
describe('Farming', () => {
  ...
  let tokenA: Contract, tokenB: Contract, tokenC: Contract
  ...
  const setupTest = deployments.createFixture(async ({ deployments, ethers }) => {
    ...
    const basicTokenFactory = await ethers.getContractFactory('vETH1')
    tokenA = await basicTokenFactory.deploy()
    tokenB = await basicTokenFactory.deploy()
    tokenC = await basicTokenFactory.deploy()
  })
  ...
  describe('poolInfo', () => {
    describe('add', () => {
      ...
      it('fails for invalid rewardPerBlock', async () => {
        await expect(farming.add(tokenA.address, [tokenB.address, tokenC.address], [100], 0, 10)).to.be.revertedWith(
          'INVALID_REWARDS'
        )
      })
    ...
    describe('set', () => {
      ...
      it('set with update and not with update', async () => {
        ...
        await tokenA.mint(wallet0.address, 400)
    ...
    describe('charge', () => {
      ...
      beforeEach('deploy', async () => {
        farming = await farmingFactory.deploy()
        await farming.initialize()
        await tokenB.mint(wallet0.address, 400)
        await tokenB.approve(farming.address, constants.MaxUint256)
        await tokenC.mint(wallet0.address, 400)
      ...
      it('successful charge and check balance', async () => {
        ...
        expect(await farming.getRemaingRewards(0)).to.deep.eq([BigNumber.from(200), BigNumber.from(400)])
      })
    })
    ...
    describe('updatePool', () => {
      ...
      beforeEach('deploy', async () => {
        farming = await farmingFactory.deploy()
        await farming.initialize()
        await tokenB.mint(wallet0.address, 1000)
        await tokenB.approve(farming.address, constants.MaxUint256)
        await tokenC.mint(wallet0.address, 1000)
        await tokenC.approve(farming.address, constants.MaxUint256)
      })
      ...
      it('update accRewardPerShare if farmingSupply larger than zero', async () => {
        ...
        await tokenA.mint(wallet0.address, 1000)
        await tokenA.approve(farming.address, constants.MaxUint256)
        await farming.stake(0, tokenA.address, 400)
        await time.advanceBlockTo(108)
        await farming.updatePool(0)
    ...
    describe('stake', () => {
      ...
      describe('one account', () => {
        ...
        beforeEach('deploy', async () => {
          ...
          await tokenA.mint(wallet0.address, 1000)
          await tokenA.approve(farming.address, constants.MaxUint256)
          await tokenB.mint(farming.address, 10000)
          await tokenC.mint(farming.address, 20000)
        })
        ...
        it('init userInfo when first stake', async () => {
          ...
          const poolInfo = parsePoolInfo(await farming.getPoolInfo(0))
          const expectRewardDebt = poolInfo.accRewardPerShare.map((share) => share.mul(BigNumber.from(200)).div(1e12))
        ...
        it('correctly update pending and rewardDebt', async () => {
          ...
          const expectRewardDebtAfterFirstStake = poolInfoFirstStake.accRewardPerShare.map((share) =>
            share.mul(BigNumber.from(300)).div(1e12)
          )
          ...
          expect(poolInfoSecondStake.accRewardPerShare).to.be.deep.eq([
            BigNumber.from(333_333_333_333),
            BigNumber.from(666_666_666_666),
          ])
          const expectRewardDebtAfterSecondStake = poolInfoSecondStake.accRewardPerShare.map((share) =>
            share.mul(BigNumber.from(700)).div(1e12)
          )
          const expectPendingAfterSecondStake = poolInfoSecondStake.accRewardPerShare.map((share, i) =>
            BigNumber.from(300).mul(share).div(1e12).sub(expectRewardDebtAfterFirstStake[i])
          )
          // only one person should take all rewards: (142 - 141) * [100, 200] => [99, 199]
          expect(userInfoSecondStake.pending).to.be.deep.eq([BigNumber.from(99), BigNumber.from(199)])
      ...
      describe('two accounts', () => {
        ...
        beforeEach('deploy', async () => {
          ...
          await tokenA.mint(wallet0.address, 1000)
          await tokenA.approve(farming.address, constants.MaxUint256)
          await tokenA.mint(wallet1.address, 1000)
  ...
  describe('redeem', () => {
    ...
    describe('one account', () => {
      ...
      it('redeem half of staked amount', async () => {
        ...
        const userInfo = parseUserInfo(await farming.getUserInfo(0, wallet0.address))
        const poolInfo = parsePoolInfo(await farming.getPoolInfo(0))
        const expectRewardDebt = poolInfo.accRewardPerShare.map((share) => share.mul(BigNumber.from(100)).div(1e12))
        expect(userInfo).to.deep.eq({
      ...
      it('correctly update when stake and redeem', async () => {
        ...
        const userInfo = parseUserInfo(await farming.getUserInfo(0, wallet0.address))
        const poolInfo = parsePoolInfo(await farming.getPoolInfo(0))
        const expectRewardDebt = poolInfo.accRewardPerShare.map((share) => share.mul(BigNumber.from(400)).div(1e12))
        expect(userInfo).to.deep.eq({
          amount: BigNumber.from(400),
          rewardDebt: expectRewardDebt,
  ...
  describe('setClaimableBlock', () => {
    ...
    it('successful set and check poolInfo', async () => {
      await farming.add(tokenA.address, [tokenB.address, tokenC.address], [100, 200], 10, 10)
      await expect(farming.setClaimableBlock(0, 0)).to.emit(farming, 'ClaimableBlockUpdated').withArgs(0, 0)
      const poolInfo = parsePoolInfo(await farming.getPoolInfo(0))
      expect(poolInfo).to.deep.eq({
        farmingToken: tokenA.address,
        amount: BigNumber.from(0),
